...
public class ViseApi {
     ...
     * @return
     */
    public <T> Subscription post(final String url,final String token, final Map<String, String> maps,ApiCallback<T> callback) {
        return this.post(url,token, maps,ClassUtil.getTClass(callback)).subscribe(new ApiCallbackSubscriber(context,callback));
    }
    ...
    public <T> Observable<T> uploadImage(String url, final String token, File file, Class<T> clazz) {

        return apiService.uploadImage(url,token, RequestBody.create(okhttp3.MediaType.parse("image/jpg;"), file)).compose
                (this.norTransformer(clazz));
    }
    public <T> Subscription uploadImage(final String url,final String token,final File file,ApiCallback<T> callback){
        return this.uploadImage(url, token, file,ClassUtil.getTClass(callback)).subscribe(new ApiCallbackSubscriber(context,callback));
    }
     ...
     * @return
     */
    public <T> Subscription uploadFile(String url, final String token, RequestBody description, MultipartBody.Part body,ApiCallback<T> callback) {
        return this.uploadFile(url,token, description, body,ClassUtil.getTClass(callback)).subscribe(new ApiCallbackSubscriber(context,callback));
    }
    /**
     * 上传文件
     *
     ...
     * @return
     */
    public <T> Subscription apiPost(final String url,final String token, final Map<String, String> parameters, ApiCallback<T> callback) {
        return this.apiPost(url,token, parameters, ClassUtil.getTClass(callback)).subscribe(new ApiCallbackSubscriber(context, callback));
    }
    /**
     * 返回ApiResult<T>的POST方式请求，无需订阅，只需传入Callback回调
     *
